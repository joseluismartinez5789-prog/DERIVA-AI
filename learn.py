...
def cargar_diagnostico():
    ruta = Path("data/diagnostico.json")
...
datos_diagnostico = cargar_diagnostico()
nivel = obtener_nivel()
...
siguiente_tema = ruta_personalizada[0] if ruta_personalizada else None
...
for indice, tema in enumerate(ruta_personalizada, start=1):
    ...
    estado = "Comienza aquí" if indice == 1 else "Pendiente"
