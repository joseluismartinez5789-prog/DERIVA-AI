...
preguntas = [
    ...
    {
        "tipo": "texto",
        "pregunta": "Calcula la derivada de f(x)=x²",
        "opciones": [
            "x",
            "2x",
            "x²",
...

    {
        ...
        "opciones": [
            "Un límite cuando Δx se aproxima a cero",
            "Una suma de funciones",
            "Una multiplicación directa",
            "Una ecuación cuadrática"
        ],
        "respuesta": "Un límite cuando Δx se aproxima a cero",
...
def _mostrar_resultado_guardado(resultado):
    st.title("📊 Revisión de la evaluación diagnóstica")

    nivel = resultado.get("nivel", "Sin diagnosticar")
    puntaje = resultado.get("puntaje", 0)
    total = resultado.get("total", len(preguntas))

    st.success(
        "Esta evaluación está guardada permanentemente "
        "en tu cuenta."
    )
    ...
    with col1:
        ...
        if fortalezas:
            ...
            st.write(
                "Todavía no se identificaron fortalezas específicas."
            )

    with col2:
        st.subheader("📌 Aspectos por reforzar")
        if mejorar:
            for area in mejorar:
                st.write(f"📌 {area}")
        else:
            st.write(
                "Excelente dominio del contenido evaluado."
            )

    plan_estudios = resultado.get(
        "plan_estudios",
        [],
    )
    ...
    if plan_estudios:
        st.divider()
        st.header("🧭 Plan de estudios personalizado")

        for modulo in plan_estudios:
            orden = modulo.get("orden", "")
            area = modulo.get("area", "Área")
    ...
    siguiente_leccion = resultado.get(
        "siguiente_leccion"
    )
    siguiente_area = resultado.get(
        "siguiente_area"
    )
    ...
    if not detalle:
        ...
        for indice, pregunta in enumerate(preguntas):
            respuesta_usuario = respuestas.get(
                str(indice),
    ...
    for posicion, item in enumerate(
        ...
        with st.expander(
            ...
            st.write(
                item.get("respuesta_usuario")
                or "Sin respuesta registrada."
            )
    ...
    st.page_link(
        "pages/learn.py",
        label="🚀 Continuar mi ruta de aprendizaje",
        icon="📚",
        use_container_width=True,
    )


estudiante_id_actual = st.session_state.get(
    "estudiante_id"
)
...
if (
    estudiante_id_actual
    and diagnostico_completado()
    and not resultado_guardado
    and not st.session_state.get(
    ...
    st.warning(
        "Tu cuenta indica que ya realizaste el diagnóstico, "
        "pero la evaluación anterior fue guardada con la versión "
        "antigua y no contiene las respuestas para revisarlas."
    )

    st.info(
        "Para crear el registro permanente debes realizar la "
        "evaluación una vez más. Después de hacerlo, el resultado "
        "quedará disponible incluso si cierras sesión."
    )

    if st.button(
        "📝 Realizar evaluación y guardar resultado",
        use_container_width=True,
    ):
...
st.markdown(
    f"""<div class="diagnostic-question-card"><div class="diagnostic-question-step">Pregunta {indice + 1} de {len(preguntas)}</div><div class="diagnostic-question-category">📘 {pregunta["categoria"]}</div><div class="diagnostic-question-text">{pregunta["pregunta"]}</div></div>""",
    unsafe_allow_html=True
)
